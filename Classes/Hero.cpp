...
//Change
//void Hero::qianqianCallback(CCObject* obj)
//{
//    isqianqian = false;
//}
...
void Hero::setAction(int actionID,int times)
{
    ...
    if (this->actionID!=actionID)
    {
       ...
       // this->animation->setMovementEventCallFunc(this, SEL_MovementEventCallFunc(&Hero::actionCallback));    //动画完成后的回调
		this->animation->setMovementEventCallFunc(this, movementEvent_selector(Hero::actionCallback));

		animation->setFrameEventCallFunc(this, frameEvent_selector(Hero::collison));
        if(times == 1 ||times == 0)
        {
...
{
    if (!KingData->gettmpshoushen() && !this->isDie)
    {
        this->stand();
        return;
    }
...
void Hero::skill4(CCObject* pSender)
{
    ...
    if(KingData->gettmpjinjiDuobi() == false)
    {
      //  gameLayer->shop();
        return;
    }
...
CCArmature* Hero::addMotion(CCAction* act,float time)
{
    ...
    s2->runAction(CCSequence::create(CCFadeOut::create(time),CCCallFuncN::create(this, callfuncN_selector(Hero::removeMotion)),NULL) );
    if(s2!=NULL)
