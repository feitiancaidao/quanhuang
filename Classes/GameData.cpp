...
void GameData::alterHEROATK()
{
    HEROATK = 50 + (this->getGongjiLevel()-1)*this->getGongjiLevel()*30;
           CCLOG( "0getIntegerForKey%d",CCUserDefault::sharedUserDefault()->getIntegerForKey("heroAtk") );
    ...
    CCUserDefault::sharedUserDefault()->purgeSharedUserDefault();
        ...
        CCLOG( "1getIntegerForKey%d",CCUserDefault::sharedUserDefault()->getIntegerForKey("heroAtk") );
    if (Role!=NULL)
    {
        Role->setAttackP(HEROATK);
        Role->setAttackPP(HEROATK);
        
               CCLOG( "Role->setAttackP%d",Role->getAttackP() );
    }
}
...
bool GameData::init()
{
    ...
    }else
    {
		...
		firstFuHuo = CCUserDefault::sharedUserDefault()->getBoolForKey("firstFuHuo", firstFuHuo);
		...
		tmpshoushen = shoushen;
		tmpjinjiDuobi = jinjiDuobi;
		tmpbazhinv = bazhinv;
		tmpbajiubei = bajiubei;
    }
    return true;
}

void GameData::reLoadSkills()
{
	jinjiDuobi = CCUserDefault::sharedUserDefault()->getBoolForKey("jinjiduobi",false);
	shoushen = CCUserDefault::sharedUserDefault()->getBoolForKey("shoushen", false);
	bazhinv = CCUserDefault::sharedUserDefault()->getBoolForKey("bazhinv", false);
	bajiubei = CCUserDefault::sharedUserDefault()->getBoolForKey("bajiubei", false);
...
void GameData::flush()
{
	...
	CCUserDefault::sharedUserDefault()->setBoolForKey("firstFuHuo", firstFuHuo);
