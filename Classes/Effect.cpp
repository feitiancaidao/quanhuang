...
bool Effect::init(const char* name,float delay,unsigned int isLoop)
{
    ...
    CCCallFunc* callfunc = CCCallFunc::create((Effect*)this, callfunc_selector(Effect::remove));
...
CCAnimation* Effect::createAnimation(const char* name,float delay, unsigned int isLoop)
{
    ...
    int index = 0;
    do {
       // const char* spriteFileName = CCString::createWithFormat("%s%d.png",name,index++)->getCString();
