...
public:
	...
	CC_SYNTHESIZE(bool, tmpjinjiDuobi, tmpjinjiDuobi);
	CC_SYNTHESIZE(bool, tmpbazhinv, tmpbazhinv);
	CC_SYNTHESIZE(bool, tmpbajiubei, tmpbajiubei);
	CC_SYNTHESIZE(bool, tmpshoushen, tmpshoushen);


	void setFirstFuHuo(bool value)
	{
		firstFuHuo = value;
		CCUserDefault::sharedUserDefault()->setBoolForKey("firstFuHuo", firstFuHuo);
	}
	bool getFirstFuHuo()const
	{
		return firstFuHuo;
	}
	...
	CC_SYNTHESIZE(CCLayer*, _GameLayer, GameLayer);
