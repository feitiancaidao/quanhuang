...
//        this->removeFromParent();
    ...
    if(this->type == 1)
    {
        int attackpp =   attcker->attackP*(1-0.08*this->getDP());
        
        CCLOG("this->dp%d",this->getDP());
        
        CCLOG("attackpp%d",attackpp);
...
void ARPG::attackJiangzhi()
{
    ...
    CCCallFunc* call = CCCallFunc::create((GameScene*)this, callfunc_selector(GameScene::resumeJiangzhi));
...
void ARPG::beAttackCallFuncO(CCObject* pSender)
{
    ...
    {
        CCCallFunc* call = CCCallFunc::create((Hero*)this, callfunc_selector(Hero::daodiqishen));
        this->runAction(call);
    }
}
